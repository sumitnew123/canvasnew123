    ...
    G = 
{
	...
	{			
		...
		$.each(data.data, function(key, value){		
			...
			var $img = '<img src="'+value.images.standard_resolution.url.replace('https','http')+'" full="'+value.images.standard_resolution.url+'" color="" />';
    ...
    updateAllInstagramImage : function(html,url,id,token){
        $('.instagram-signin, .pdc_instagram_act').hide();
        $('.pdc_instagram').show();
    ...
    show_ins_images: function(id){
        ...
        $.ajax({
    		...
    		success: function(data) {
    		 ...
    		 for(i=0;i<data.data.length;i++){
    		   $('#pdp_ins_feed_friends_image_list').prepend('<img src="'+data.data[i].images.standard_resolution.url.replace('https','http')+'" user="'+id+'" color="" />')
                //follows.push(data.data[i]);
             }
    		 if(data.pagination.next_url!=undefined){
    ...
    show_ins_images_next: function(url,id){
        ...
        $.ajax({
    		...
    		success: function(data) {
    		  $('#pdp_ins_feed_friends .loading-img').hide();
    		 for(i=0;i<data.data.length;i++){
    		   $('#pdp_ins_feed_friends_image_list').append('<img src="'+data.data[i].images.standard_resolution.url.replace('https','http')+'" user="'+id+'" color="" />')
                //follows.push(data.data[i]);
             }
    		 if(data.pagination.next_url!=undefined){
    		      $('.pdp_ins_friends_images_next').show();
    ...
    $('.instagram-signout').click(function(){
        ...
        $('.instagram-signin, .pdc_instagram_act').show();
        $(this).hide();
    ...
    $('.pdc_instagram').on('click', '.pdp_ins_tab', function(){
