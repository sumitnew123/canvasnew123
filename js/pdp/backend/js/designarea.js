        ...
        previewImageDesignArea : function(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (e) {
                    ...
                    $('#thumbnail_background_image').attr('src', e.target.result);
                    $('#thumbnail_background_image').show();
                }
		...
		previewOverImage : function(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (e) {
                    ...
                    $('#thumbnail_preview_mask_photo').attr('src', e.target.result);
                    $('#thumbnail_preview_mask_photo').show();
                }
                reader.readAsDataURL(input.files[0]); 
            }
        },
        previewMaskPhoto : function(input) {
            if (input.files && input.files[0]) {
                var reader = new FileReader();
                var designObj = $('#thumbnail_preview_mask_photo');
                reader.onload = function (e) {
                	designObj.attr('src', e.target.result);
                    $('#thumbnail_preview_mask_photo' ).show();
                }
                reader.readAsDataURL(input.files[0]);
            }
        ...
        switchBackgroundType: function() {
            ...
            $('#canvassize').change(function(){
                if($(this).val() == 'custom_size'){
                    $('#pdpcanvaswidth').val('');
                    $('#pdpcanvasheight').val('');
                    $('#pdpcanvaswidth').focus();
                    $('.canvas-size').show();
                }else{
                    $('.canvas-size').show();
                    var size = $(this).val();
                    var res = size.split("x");
                    $('#pdpcanvaswidth').val(res[0]);
                    $('#pdpcanvasheight').val(res[1]);
                    $('.pdp-img-area').css('width',res[0]+'px');
                    $('.pdp-img-area').css('height',res[1]+'px');
                }
            });
            $('#pdpcanvaswidth').keyup(function(){
                $('.pdp-img-area').css('width',$(this).val()+'px');
            });
            $('#pdpcanvasheight').keyup(function(){
                $('.pdp-img-area').css('height',$(this).val()+'px');
            });     
            $('.pdp-img-area').css('width',$('#pdpcanvaswidth').val()+'px');
            $('.pdp-img-area').css('height',$('#pdpcanvasheight').val()+'px');
            if($('#use_mask').val() == '1'){
                $('.upload_photo_mask').show();
                $('#overlay_image').show();
            }else{
                $('.upload_photo_mask').hide();
                $('#overlay_image').hide();
            }            
            $('#use_mask').change(function(){
                if($(this).val() == '1'){
                    $('.upload_photo_mask').show();
                    $('#overlay_image').show();
                }else{
