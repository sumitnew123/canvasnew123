;(function(global, $) {
    // 'new' an object
    var setdefaultlayer = true; // added by shrikant
    var sideCounter = 0; // added by shrikant
    var flexproductId = $("#current_product_id").val();
    var mynewflexvalue;
    //var inHeight = document.getElementById("flexheightInput").value;
    //var inWidth = document.getElementById("flexwidthInput").value;
    var rHeight = 700;
    var rWidth = 400;
    var flagcheck = localStorage.flagckh;

    if(flagcheck = true){
        rHeight = localStorage.newHeight * 70;
        rWidth = localStorage.newWidth  * 70;
        //alert(rHeight);
        //alert(rWidth);
    }
    
    
    var flexjson = '{"6":{"id":"6","product_id":"23","label":"Banner","color_id":"0","inlay_w":"4252","inlay_h":"3402","inlay_t":"0","inlay_l":"0","filename":"filename1479409605.png","position":"0","status":"1","price":"0.00","overlay":"","color_code":"","color_name":"","background_type":"blank","canvassize":"'+ rWidth +'x'+ rHeight +'","canvaswidth":"'+ rWidth +'","canvasheight":"'+ rHeight + '","use_mask":"2","thumbnail":"resize\/resize_filename1479409605.png"}}';

    function setflexwrapper() {
       rHeight = $(".pdc-product-image").height();
       rWidth = $(".pdc-product-image").width();
       rHeight = rHeight + 17;
       rWidth = rWidth + 17;
       //alert(rWidth);
       $(".pdc-product-image").height(rHeight);
       $(".pdc-product-image").width(rWidth);
       //alert($(".pdc-product-image").height());
       $(".pdc-product-image").css("padding-top", "15px");
       $(".pdc-product-image").css("padding-left", "15px");
       $(".pdc-product-image").css("border", "1px dashed #999999");
       $(".pdc-product-image").css("background", "url(http://dev.bulbandkey.com/js/pdp/pdc/images/ruler.png) no-repeat");

       $(".flex-tool-tab").css("display", "inline-block");

    }

    
    localStorage.setItem("newflexdata", flexjson);

    //var fxjs=JSON.stringify(localStorage.newdata);
    //var flexjason = JSON.parse(fxjs);
    mynewflexvalue = localStorage.newflexdata;

    $(document).ready(function() {
        $("#applyFlexDimentions").click(function(){
            rHeight = document.getElementById("flexheightInput").value;
            rWidth = document.getElementById("flexwidthInput").value;
            var cnJson = document.getElementById("sides_config").value;

            localStorage.setItem("flagckh", true);

            localStorage.setItem("newHeight", rHeight);
            localStorage.setItem("newWidth", rWidth);
            location.reload();
            
            var flexjson = '{"6":{"id":"6","product_id":"23","label":"Banner","color_id":"0","inlay_w":"4252","inlay_h":"3402","inlay_t":"0","inlay_l":"0","filename":"filename1479409605.png","position":"0","status":"1","price":"0.00","overlay":"","color_code":"","color_name":"","background_type":"blank","canvassize":"'+ inWidth +'x'+ inHeight +'","canvaswidth":"'+ inWidth +'","canvasheight":"'+ inHeight + '","use_mask":"2","thumbnail":"resize\/resize_filename1479409605.png"}}';

           localStorage.setItem("newflexdata", flexjson);
            
            //(flexjason.[6].canvaswidth);
           // var text = flexjason.['6'].canvaswidth;
           // flexjason.canvassize = "200x200";
            //flexjason.canvaswidth = "200";
            //flexjason.canvasheight = "200";
            //vat newobj = Object.byString(flexjason, 'canvaswidth');
            //console.log(newobj);
            //localStorage.setItem("newdata", JSON.stringify(flexjason));
    ...
    if(flexproductId == 23){
        $("#sides_config").val(localStorage.newflexdata);
    }
	...
	PDC.prototype = {
            ...
            if(self.checkJSONReady.status == 1) {
                ...
                self.saveJsonFile(_saveSampleOptions, function(response) {
                    ...
                    if(responseJSON.status == "error") {
                    	...
                    	if(responseJSON.message == "guest") {
                        	self.showLog("Saved design in guest Account", "warn"); 
                        }
        ...
        initCanvas: function() {
            ...
            if(activeSide) {
                ...
                if(sideConfig) {
                    ...
                    // Added By Shrikant to change sides change color in one time
                    this.changeBackgroundColor(globalBgColor);
                    
                    //this.setActiveObject();
                    /*if(setdefaultlayer){
                        this.addDefaultImage('http://dev.bulbandkey.com/media/pdp/images/artworks/resize/resize_Classic/artwork1441234203.png');
                        sideCounter--;
                        if(sideCounter == 0){
                            setdefaultlayer = false;
                        }
                    }*/
                    //Show canvas size
        ...
        addDefaultImage: function(src, options, callback) {
            options = options || {};
            if(src) {
                var ext = src.split("."),
                ...
                if(ext[ext.length -1] != 'svg') {
                    ...
                    fabric.Image.fromURL(src, function(image) {
                        ...
                        //Compare image size with original canvas size
                        /*if((image.width > (_canvas.width / _canvas.scale)) || (image.height  > (_canvas.height / _canvas.scale))) {
                            if(image.width > image.height) {
                                image.scaleToWidth(_canvas.width + 40);
                            } else {
                                image.scaleToHeight(_canvas.height +40);
                            }
                        } else { */
                            image.setWidth(image.width * _canvas.width);
                            image.setHeight(image.height * _canvas.width);
                        //}
                        image.setCoords();
                ...
                if(ext[ext.length -1] != 'svg') {
                    ...
                    fabric.Image.fromURL(src, function(image) {
                        ...
                        if((image.width > (_canvas.width / _canvas.scale)) || (image.height  > (_canvas.height / _canvas.scale))) {
                            ...
                            image.setWidth(image.width * _canvas.scale);
                            image.setHeight(image.height * _canvas.scale);
                        }
                        image.setCoords();
                        _canvas.centerObject(image);
                        _canvas.add(image).setActiveObject(image);
        ...
        closeIframe: function() {
            ...
            $("#pdc_iframe", top.document).hide();
            $("#create-project-section", top.document).show();
        ...
        pdcZoom: {
            ...
            zoomIn: function(_canvas, SCALE_FACTOR) {
                ...
                //added by shrikant for flex tool
                if(flexproductId == 23){
                    setflexwrapper();
                }
            ...
            updateWrappSize: function(_canvas) {
                if(!_canvas) return false;
                var activeSideId = $(".side-item.active").attr("id").replace("side-", ""),
                    canvasSideId = _canvas.lowerCanvasEl.id.replace("canvas_side_", "");
                //Only update wrapper in active side, fix update wrap size in each loop
                if(activeSideId != canvasSideId) return false;
                $('[pdc-data="main-canvas"]').css({
                    width: _canvas.width,
    ...
    PDC.init = function() {
        ...
        $('[pdc-data="add-text"]').click(function() {
            ...
            $(".pdc-style-family").prop("selectedIndex", 0).css({"font-family": "Arial"});
        });
        //**************************** End X3 Event ******************************
        // Add event listener
        $('[pdc-action="SAVE_SAMPLE_DESIGN"]').click(function() {
            if(self.checkJSONReady.status == 1) {
                ...
                if(_canvas) {
                    _canvas.clone(function(_canvasTemp) {
                        if(_canvasTemp.width > maxWidth) {
                            var zoom = maxWidth / _canvasTemp.width;
                            _canvasTemp.setZoom(zoom);
                            _canvasTemp.setWidth(_canvas.width * zoom);
                            _canvasTemp.setHeight(_canvas.height * zoom);
                        } 
                        var src = _canvasTemp.toDataURL({
                                format: 'jpeg',
                                quality: 1
                            });
                        self.doRequest( config.save_thumbnail_url, {
                            ...
                            if(jsonResponse.status == "success") {   
                                ...
                                var _saveSampleOptions = {
                                    action: 'save_sample',
                                    product_id: self.getCurrentProductId(),
                                    template_thumbnail: jsonResponse.filename,
                                ...
                                self.saveJsonFile(_saveSampleOptions, function(response) {
                                    ...
                                    var responseJSON = JSON.parse(response);
                                    if(responseJSON.status == "error") {
                                        self.showLog("Can not save this design", "warn");
                                        return false;
                                    } else {
                                        self.newestJsonFilename = responseJSON.filename;
                                        try {
                                            //console.info("Redirect to view template library page");
                                            var	url = $("#view_design_template_url", top.document).val() + "productid/" + self.getCurrentProductId();
                                            window.location = url;
                                        } catch (error) {
                                            console.error(error);
                                        }
                ...
                self.saveDesignToJSON(null, $(this));
            }
        });
        $('[pdc-action="REMOVE_SAMPLE_DESIGN"]').click(function() {
            self.removeSampleData();
        });
        $('[pdc-data="SAVE_GUEST_DESIGN"]').click(function() {
            if(self.checkJSONReady.status == 1) {
                //Ready to save json
                var _saveSampleOptions = {
                    action: 'save_guest_design',
                    product_id: self.getCurrentProductId()
                }
                self.saveJsonFile(_saveSampleOptions, function(response) {
                    var responseJSON = JSON.parse(response);
                    if(responseJSON.status == "error") {
                        self.showLog("Can not save guest design", "warn");
                        return false;
                    } else {
                        self.newestJsonFilename = responseJSON.filename;
                        //Reset checking after 
        ...
        $('[pdc-data="SAVE_CUSTOMER_DESIGN"]').click(function() {
        	self.saveDesignToCustomerAccount();
        	/*var isCustomerLogin = $("#is_customer_logged").val() || 0;
            if(isCustomerLogin) {
            	self.saveDesignToCustomerAccount();
            	*//*
                //Try to save json to customer account
                if($("#pdp-save-design-details").length) {
                     //Show design details modal
                    $.fancybox({
                        href: '#pdc-save-design-details', 
                        modal: false,
                    });
                    return false;   
                } else {
        ...
        $('[pdc-data="RESET_DESIGN"]').click(function() {
            self.resetToSampleDesign();
        });
        $('[pdc-data="SAVE_DESIGN_BEFORE_CLOSE"]').click(function() {
            $('[pdc-data="SAVE_GUEST_DESIGN"]').click();
        });
        $('[pdc-data="CLOSE_DESIGN_WITHOUT_SAVE"]').click(function() {
            self.closeIframe();
        });
        $('[pdc-data="SHARE_DESIGN"]').click(function() {
            self.exportThumbnailImageForShare();
        });
        $('[pdc-data="VIEW_TEMPLATE_LIBRARY"]').click(function() {
            var	url = $("#view_design_template_url", top.document).val() + "productid/" + self.getCurrentProductId();
            window.location = url;
        });
        $('[pdc-data="DOWNLOAD-PNG"]').click(function() {
            self.downloadPng();
        });
        $('[pdc-data="DOWNLOAD-PDF"]').click(function() {
            self.downloadPdfFromPng();
        });
        $('[pdc-data="SAVE_CUSTOMER_DESIGN_DETAILS"]').click(function() {
            self.saveDesignWithDetailsToCustomerAccount();
        });
        //Change product color 
        $('[pdc-background-images]').click(function() {
            var images = $(this).attr("pdc-background-images");
            $('.product-colors li').removeClass("active");
            $(this).addClass("active");
            if(images) {
                images = JSON.parse(images);
                //Update background + overlay to _sideConfig
                $.each(_sidesConfig, function(index, _side) {
                    (function(side) {
                        side.filename = images[side.id].filename;
                        side.overlay = images[side.id].overlay;
                        //Update side thumbnail
                        var bgImage = config.media_url + side.filename,
                            mask = config.media_url + side.overlay;
                        //Need to update background path also
                        side.background_path = bgImage;
                        //Use thumbnail if exists 
                        var bgThumbnail = bgImage;
                        if(images[side.id].filename_thumbnail) {
                            bgThumbnail = config.media_url + images[side.id].filename_thumbnail;
                        }
                        $("#side-" + side.id).find("img").attr("src", bgThumbnail);
                        self.addBackgroundLayer(bgImage, {
                            object_type: "background", 
                            side_id: side.id,
                            skip_update_background_path: true //already update side.background_path on this loop, so skip update on: updateBackgroundInfoToSizeConfig
                        }, self.allCanvas[side.id]);
                        self.addOverlayLayer(mask, {}, self.allCanvas[side.id]);
                        self.allCanvas[side.id].renderAll();
                    })(_side);
                });
                //Re-render background and overlay
            }
        });
        self.prepareCanvas();
        self.initCanvas();
