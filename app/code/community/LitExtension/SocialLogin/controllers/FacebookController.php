...
class LitExtension_SocialLogin_FacebookController extends Mage_Core_Controller_Front_Action
{
    ...
    {
    	$facebook = new Facebook_Resource_Facebook();
    	$facebook->setAppid(Mage::getStoreConfig('le_sociallogin/facebook/api_key'));
    	$facebook->setSecret(Mage::getStoreConfig('le_sociallogin/facebook/secret'));
    	$client = $facebook->init();
    	$redirect_url = Mage::getUrl('le_sociallogin/facebook/connect');
    	$loginUrl = $client->getLoginUrl(array('scope' => 'public_profile,email,user_friends,user_birthday', 'redirect_uri' => $redirect_url ));
        ...
        $isapp = $this->getRequest()->getParam('isapp', 0);
        Mage::getSingleton('core/session')->setApplogin($isapp);
    ...
    public function connectAction()
    {
        ...
        $isapplogin = Mage::getSingleton('core/session')->getApplogin();
	    if(Mage::getSingleton('customer/session')->isLoggedIn() and $isapplogin){
			$this->_redirect( '*/index/customer', array('id'=>Mage::getSingleton('customer/session')->getCustomer()->getId()) );
	    } else {
    ...
    protected function _connectCallback()
    {
        ...
        if ($code) {
	    	...
	    	$facebook->setAppid(Mage::getStoreConfig('le_sociallogin/facebook/api_key'));
	    	$facebook->setSecret(Mage::getStoreConfig('le_sociallogin/facebook/secret'));
	    	$fclient = $facebook->init();
    		$userdata = $fclient->getUserData();
    		//echo '<pre>'; print_r($userdata); exit;
    		
            //$userInfo = $fclient->api('/me');
            $token = $fclient->getAccessToken();
			$userInfo = new Varien_Object($userdata);
