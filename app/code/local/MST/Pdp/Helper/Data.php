...
class MST_Pdp_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	{
        return true; //Shouldn't delete file from server
	...
	public function getSampleJsonFile($productId) {
		$jsonFilename = Mage::getModel("pdp/admintemplate")->getDefaultDesign($productId);
		return $jsonFilename;
	}
	...
	public function getShareInfo($shareId) {
		$shareInfo = Mage::getModel('pdp/share')->load($shareId)->getData();
		return $shareInfo;
	}
	...
	public function isDesignAble($pid) {
        if(!Mage::helper("pdp")->isProductDesignAble($pid)) {
        	return false;
        }
        $productStatus = Mage::getModel('pdp/productstatus')->getProductStatus($pid);
        if ($productStatus != 1) {
            return false;
        }
        return true;
    ...
    public function isProVersion() {
        //Avaiable in pro version only
        $isPro = Mage::helper('core')->isModuleEnabled('MST_PdpPro');
        if($isPro) {
            return true;
        }
        return false;
    }
    public function isStandardVersion() {
        $isStandard = Mage::helper('core')->isModuleEnabled('MST_PdpStandard');
        if($isStandard) {
            return true;
        }
        return false;
    ...
    public function isEnterpriseVersion() {
        $isEnterpriseVersion = Mage::helper('core')->isModuleEnabled('MST_PdpStandard');
    ...
    public function getProductDesignColors($productId) {
        ...
        foreach($productColors as $_productColor) {
            $productColorDataArr[] = $_productColor->getData();
        }
        return array(
    ...
    public function isProductColorTabEnable($productId) {
        //Check all side use background image + mask image. 
        //Has product color item
        $productColors = $this->getProductDesignColors($productId);
    ...
    public function canAddProductColor($productId) {
        //Check all side use background image + mask image. 
        //Has product color item
        $productColors = $this->getProductDesignColors($productId);
        //Check default side using background and mask or not
