...
class MST_Pdp_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    {
		echo "Test Resize Image Action<br/>";
        //$uploadHelper = Mage::helper("pdp/upload");
        //$basePath = Mage::getBaseDir('media') . DS . "pdp" . DS . "images" . DS . "upload" . DS . "14478564152015-09-23_0904-custom.png";
        //Zend_Debug::dump($uploadHelper->resizeImage($basePath));
        $image1 = array(
            'thumbnail' => 'image1',
            'filename' => 'Image 1'
        );
	...
	public function loadMoreImageAction()
	{
		...
		$collection = $pdpObject->pagingCollection($current_page, $category, $pageSize, $imageType);
		if ( count($collection) > 0) {
			...
			foreach ($collection as $image) {
                ...
                $imageData['price_format'] = Mage::helper("pdp")->__("Free");
                        ...
                        if(!empty($_jsonInfo) && isset($_jsonInfo['options']['action'])) {
                            ...
                            switch($_jsonInfo['options']['action']) {
                                case 'save_sample': 
                                    ...
                                    $_template = Mage::getModel('pdp/admintemplate')->saveAdminTemplate($_tempData);
                                    if(!$_template->getId()) {
                                ...
                                case 'save_customer_design':
                                    ...
                                    $_isCustomerDesignSaved = Mage::getModel("pdp/customerdesign")->saveTemplate($_tempData);
                                ...
                                case 'save_for_share':
                                    ...
                                    $shareId = Mage::getModel('pdp/share')->saveShareData($_jsonInfo['options']);
                                    if($shareId) {
                                        $response['status'] = 'success';
    ...
    public function saveBase64ImageAction() {
       ...
       /* if(isset($data['options']['preview_thumbnail'])) {
            $previewThumbnailType = $data['options']['preview_thumbnail'];
        }*/
        $response = array(
            'status' => 'error',
    ...
    public function saveAndCreateSvgAction() {
        ...
        if(isset($data['svg_string'])) { 
            $data['svg_string'] = urldecode ($data['svg_string']);
    ...
    public function createPdfFromPngAction() {
        ...
        if(isset($data['png_string'])) { 
            ...
            if(substr($baseCode,0,4)=='data'){
                ...
                $result = $this->createImageFromString($pngString, $orderInfo, "png", $isBackend);
                if($result['file_location']) {
    ...
    public function loginAction(){
        ...
        if ($this->getRequest()->isPost()){
            ...
            if (empty($login_data['username']) || empty($login_data['password'])) {
            	$result['error'] = Mage::helper('pdp')->__('Login and password are required.');
            }else{
				try{
                    ...
                    switch ($e->getCode()) {
                        case Mage_Customer_Model_Customer::EXCEPTION_EMAIL_NOT_CONFIRMED:
                            $message = Mage::helper('pdp')->__('Email is not confirmed. <a href="%s">Resend confirmation email.</a>', Mage::helper('customer')->getEmailConfirmationUrl($login_data['username']));
                            break;
                        default:
