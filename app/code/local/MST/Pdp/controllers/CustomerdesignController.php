...
class MST_Pdp_CustomerdesignController extends Mage_Core_Controller_Front_Action
{
    ...
    public function updateDesignDetailsAction() {
        parent::preDispatch();
       	if (!Mage::getSingleton('customer/session')->authenticate($this)) {
            $this->setFlag('', 'no-dispatch', true);
	       // adding message in customer login page
        ...
        if(isset($data['title'])) {
            $data['design_title'] = $data['title'];
        }
        if(isset($data['note'])) {
            $data['design_note'] = $data['note'];
        }
        $result = Mage::getModel("pdp/customerdesign")->updateDesignDetails($data);
        if($result) {
            Mage::getSingleton('core/session')->addSuccess(Mage::helper('pdp')->__("Your design has been successfully saved"));
        } else {
            Mage::getSingleton('core/session')->addError(Mage::helper('pdp')->__("Unable to save your design"));
        }
        $this->_redirect("*/*/index");
    }
}
