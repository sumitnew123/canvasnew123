...
class MST_Pdp_ViewController extends Mage_Core_Controller_Front_Action
{
    ...
    public function viewDesignAction()
    {
        $params = $this->getRequest()->getParams();
        if(isset($params['json']) && $params['json'] != "") {
            $jsonString = $params['json'];
            $block = $this->getLayout()->createBlock('core/template')->setTemplate('pdp/review/view_design.phtml')->setData('pdpdesign', $jsonString);
            $this->getResponse()->setBody($block->toHtml());   
        } else {
            $this->norouteAction();
            return;
	...
	public function getIframeAction() {
		$block = $this->getLayout()->createBlock("pdp/x3")->setTemplate("pdp/product/iframe.phtml");
		$this->getResponse()->setBody($block->toHtml());
	}
    //*** X3 DESIGN
    public function designToolAction() {
		$this->loadLayout();
        $this->renderLayout();
	}
}