<?php
class MST_Pdp_Adminhtml_Pdpadmin_ColorController extends Mage_Adminhtml_Controller_Action {
    ...
    public function selectColorAction() {
       $this->loadLayout();
		$this->getLayout()->getBlock('color_grid')
		->setColors($this->getRequest()->getPost('colors', null));
		$this->renderLayout();
    }
    public function selectColorGridAction() {
        $this->loadLayout();
		$this->getLayout()->getBlock('color_grid')
		->setColors($this->getRequest()->getPost('colors', null));
		$this->renderLayout();
    }
    public function saveSelectedColorAction() {
        $params = $this->getRequest()->getParams();
        $response = array(
            'status' => 'error',
            'message' => 'Can not save selected color for this product!'
        );
        if (isset($params['selected_items']) && $params['productid']) {
            ...
            $result = Mage::getModel("pdp/productstatus")->saveSelectedItem($params['productid'], 'selected_color', $selectedItemsJSON);
            if($result) {
                $response = array(
                    'status' => 'success',
