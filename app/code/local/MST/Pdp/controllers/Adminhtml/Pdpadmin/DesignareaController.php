<?php
class MST_Pdp_Adminhtml_Pdpadmin_DesignareaController extends Mage_Adminhtml_Controller_Action
{
	...
	public function saveAction() {
		...
		if ($data) {
			...
			if ($image != "") {
				$data['filename'] = $image;
                //Create thumbnail for background image
                $basePath = Mage::getBaseDir('media') . DS . "pdp" . DS . "images" . DS . $image;
                ...
                $bgThumbnail = Mage::helper("pdp/upload")->resizeImage($basePath, $newPath, $_thumbOptions);
                if($bgThumbnail) {
                    $data['thumbnail'] = $bgThumbnail;
                }
			}
            ...
            $maskPhoto = Mage::helper('pdp')->saveImage('mask_photo', 'pdp/images/');
			if ($maskPhoto != "") {
				$data['mask_photo'] = $maskPhoto;
			}
            ...
            try {
				...
				if ( $main_domain != 'dev' ) { 
    				...
    				if ( $valid == false )  {
                        ...
                        $url = Mage::helper("adminhtml")->getUrl("adminhtml/pdpadmin_designarea/addNewSide/", array ("productid" => $productId));
	...
	public function saveDesignInlayAction() {
		...
		if ($filename != "") {
			...
			if ($inlayId != "") {
				...
				if ($itemId != "") {
					Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('pdp')->__('Item was successfully saved'));
					$url = Mage::helper("adminhtml")->getUrl("adminhtml/pdpadmin_designarea/setuparea/",array("productid"=> $_productId, "areaid" => $_sideId ));
					Mage::app()->getResponse()->setRedirect($url)->sendResponse();
				}
			}
		}
    ...
    public function viewDesignTemplateAction() {
        $this->initLayoutMessages('adminhtml/session');
		$block =  $this->getLayout()->createBlock('core/template')->setTemplate('pdp/product/view_design_template.phtml')->toHtml();
		$this->getResponse()->setBody($block);
	}
	public function saveDesignColorAction() {
			...
			if ($productColorId) {
				...
				foreach ($data['design_sides'] as $sideId) {
                    ...
                    if($filename != "" && $overlayFilename != "") {
                        ...
                        //Auto Create clipart thumbnail for side color
                        $basePath = Mage::getBaseDir('media') . DS . "pdp" . DS . "images" . DS . $productColorImageInfo['filename'];
                        $newPath = Mage::getBaseDir('media') . DS . "pdp" . DS . "images" . DS . "resize" . DS;
                        $_thumbOptions = array(
                            'width' => 150,
                            'height' => 150,
                        ...
                        $sideThumbnail = Mage::helper("pdp/upload")->resizeImage($basePath, $newPath, $_thumbOptions);
                        if($sideThumbnail) {
                            $productColorImageInfo['filename_thumbnail'] = $sideThumbnail;
                        }
		...
		$url = Mage::helper("adminhtml")->getUrl("adminhtml/pdpadmin_designarea/viewdesigncolor/",array("productid"=> $productColorInfo['product_id'] ));
		Mage::app()->getResponse()->setRedirect($url)->sendResponse();
	...
	public function updateDesignColorAction() {
        ...
        if(isset($data['default_side'])) {
            //( 'pdp/pdpside' )
            if(isset($data['default_side']['side_id']) && $data['default_side']['color_name'] && $data['default_side']['color_code']) {
                $sideModel = Mage::getModel("pdp/pdpside")->load($data['default_side']['side_id']);
		...
		Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('pdp')->__('Item was successfully saved'));
		$url = Mage::helper("adminhtml")->getUrl("adminhtml/pdpadmin_designarea/viewdesigncolor/",array("productid"=> $data['product_id'] ));
		Mage::app()->getResponse()->setRedirect($url)->sendResponse();
	}
    ...
    public function deleteTemplateAction() {
        $templateId = $this->getRequest()->getParam("template-id");
        $productId = $this->getRequest()->getParam("productid");
        if($templateId && $productId) {
            Mage::getModel("pdp/admintemplate")->load($templateId)->delete();
            $url = Mage::helper("adminhtml")->getUrl("adminhtml/pdpadmin_designarea/viewdesigntemplate/",array("productid"=> $productId ));
            Mage::app()->getResponse()->setRedirect($url)->sendResponse();
        }
        
    }
    public function updateDesignTemplateAction() {
        try {
            $data = $this->getRequest()->getPost();
            //Zend_Debug::dump($data);die;
            if(isset($data['design-template']) && $data['design-template']) {
                $templateModel = Mage::getModel("pdp/admintemplate");
            ...
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('pdp')->__('Design templates was successfully saved!'));
		} catch (Exception $e) {
			//Zend_Debug::dump($e);	
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('pdp')->__('Can not save this design!'));
		}
        $url = Mage::helper("adminhtml")->getUrl("adminhtml/pdpadmin_designarea/viewdesigntemplate/",array("productid"=> $data['product_id'] ));
        Mage::app()->getResponse()->setRedirect($url)->sendResponse();
    }
}