...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    {
    	$referer = Mage::getSingleton('core/session')->getCheckoutReferer();
    	if($referer) {
        ...
        if($type) {
            $this->_getSession()->setProfileType($type);
        }
        $type = $this->_getSession()->getProfileType();
        if($type == 'vendor' || (!isset($type) && Mage::helper('customer')->getCustomer()->getGroupId() == 2)) {
    ...
    public function loginAction()
    {
        ...
        if($referer) {
        	Mage::getSingleton('core/session')->setCheckoutReferer($referer);
        } else {
        ...
        if ($this->getRequest()->isPost()) {
        	...
        	try {
                ...
                $this->_getSession()->setCustomer($customer)
                    ->addSuccess($this->__('Your Password is changed successfully!.'));
