...
class Mage_Catalog_ProductController extends Mage_Core_Controller_Front_Action
{
    ...
    public function ajaxcollectionAction() {
    	$page = $this->getRequest()->getParam('page', 1);
    	$sql = Mage::getSingleton('catalog/session')->getCatalogCollectionBaseQuery();
    	$read = Mage::getSingleton( 'core/resource' )->getConnection( 'core_read' );
        $sql = str_replace('LIMIT 10', '', str_replace('LIMIT 12', '', $sql));
		$result = $read->query( $sql . ' LIMIT 10 OFFSET '.(($page-1)*10) );
		...
		<div class="collection-item">
        	<a href="<?php echo $_product->getProductUrl() ?>">
				<img src="<?php echo Mage::helper('catalog/image')->init($_product, 'small_image')->keepFrame(false)->resize(600); ?>" class="image">
				<div class="item-box-overlay-title">
					...
					<span class="overlay-content-price"><?php echo Mage::helper('core')->currency($_product->getPrice(), true, false) ?></span>
					<span class="overlay-content-description"><?php echo Mage::helper('catalog/output')->productAttribute($_product, $_product->getShortDescription(), 'short_description') ?></span>
				</div>
			</a>
		</div>
		<?php
	...
	public function ajaxdescribeAction() {
    	$page = $this->getRequest()->getParam('page', 1);
    	$sql = Mage::getSingleton('catalog/session')->getCatalogCollectionBaseQuery();
    	$read = Mage::getSingleton( 'core/resource' )->getConnection( 'core_read' );
        $sql = str_replace('LIMIT 10', '', str_replace('LIMIT 12', '', $sql));
		$result = $read->query( $sql . ' LIMIT 12 OFFSET '.(($page-1)*12) );
		//$block = new Mage_Catalog_Block_Product_List();
		while ( $row = $result->fetch() ) {
		$_product = Mage::getModel('catalog/product')->load($row['entity_id']);
		?>
			...
			<div class="container-inner">
				<div class="container-inner-section">
					<div class="describe-image">
						<a href="<?php echo $_product->getProductUrl() ?>"><img class="img-responsive" src="<?php echo Mage::helper('catalog/image')->init($_product, 'small_image')->keepFrame(false)->resize(600); ?>" alt="<?php echo $_productNameStripped ?>"></a>
					</div>
