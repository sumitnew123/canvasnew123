...
abstract class Mage_Sales_Model_Order_Pdf_Items_Abstract extends Mage_Core_Model_Abstract
{
        ...
        if (Mage::helper('tax')->displaySalesBothPrices()) {
            $prices = array(
                array(
                    ...
                    'price'    => $this->formatPriceTxt($order, $item->getPrice()),
                    'subtotal' => $this->formatPriceTxt($order, $item->getRowTotal())
                ),
                array(
                    'label'    => Mage::helper('tax')->__('Incl. Tax') . ':',
                    'price'    => $this->formatPriceTxt($order, $item->getPriceInclTax()),
                    'subtotal' => $this->formatPriceTxt($order, $item->getRowTotalInclTax())
                ),
            );
    ...
    public function formatPriceTxt($order, $price) {
    	if($order->getBaseCurrencyCode() == 'INR') {
    		return 'Rs. '. Mage::getModel('directory/currency')->format($price, array('display'=>Zend_Currency::NO_SYMBOL), false);
    	} else {
    		return $order->formatPriceTxt($price);
    	}
    }
