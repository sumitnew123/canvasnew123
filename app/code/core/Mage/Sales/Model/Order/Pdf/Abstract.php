...
abstract class Mage_Sales_Model_Order_Pdf_Abstract extends Varien_Object
{
	...
	protected function insertVendor(&$page, $vendor = null)
    {
        $this->y = $this->y ? $this->y : 815;
        $image = $vendor->getLogo();
        if ($image) {
            $image = Mage::getBaseDir('media') . '/vendor/image/' . $image;
            if (is_file($image)) {
                $image       = Zend_Pdf_Image::imageWithPath($image);
                $top         = 830; //top border of the page
                ...
                $page->drawText($vendor->getTitle(), $x1, $y1 - 10, 'UTF-8');
    ...
    protected function insertOrder(&$page, $obj, $putOrderId = true)
    {
        ...
        $page->drawText(strip_tags(trim($order->getPayment()->getMethodInstance()->getTitle())), $paymentLeft, $yPayments, 'UTF-8');
        $yPayments -= 15;
        ...
        if ($order->getIsVirtual()) {
            ...
            if($order->getBaseCurrencyCode() == 'INR') {
	    		$totalShippingChargesText = "(" . Mage::helper('sales')->__('Total Shipping Charges') . " "
                . 'Rs. '. Mage::getModel('directory/currency')->format($this->getAmount(), array('display'=>Zend_Currency::NO_SYMBOL), false) . ")";
	    	} else {
	    		$totalShippingChargesText = "(" . Mage::helper('sales')->__('Total Shipping Charges') . " "
