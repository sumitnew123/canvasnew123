...
class Mage_Checkout_CartController extends Mage_Core_Controller_Front_Action
{
    ...
    public function indexAction()
    {
    	if(!Mage::helper('customer')->isLoggedIn()) {
    		...
    		$this->_redirect('customer/account/login', array('referer' => Mage::helper('core')->urlEncode(Mage::getUrl("checkout/cart/"))));
			return;
    	}
        ...
        if ($cart->getQuote()->getItemsCount()) {
        	...
        	$this->_getSession()->addError('No product to Purchase');
        	$this->_redirect('collections.html');
			return;
        }
        ...
        try {
            ...
            if (!$this->_getSession()->getNoCartRedirect(true)) {
                if (!$cart->getQuote()->getHasError()) {
                    $message = $this->__('%s / to your shopping cart.', Mage::helper('core')->escapeHtml($product->getName()));
    ...
    public function deliveryAction() {
    	Mage::getSingleton('core/session')->setCheckoutReferer(false);
    	$this
            ->loadLayout()
            ->_initLayoutMessages('checkout/session')
            ->_initLayoutMessages('catalog/session')
            ->getLayout()->getBlock('head')->setTitle($this->__('Delivery Details'));
        $this->renderLayout();
    }
