...
class Mage_Checkout_OnepageController extends Mage_Checkout_Controller_Action
{
    ...
    public function placeorderAction() {
        ...
        $quote = $this->getOnepage()->getQuote();
        $sales_address = Mage::getSingleton('sales/quote_address');
        $quote->setBillingAddress($sales_address->importCustomerAddress($address));
		$quote->setShippingAddress($sales_address->importCustomerAddress($address));
    	$quote->save();
    	...
    	$result = $this->getOnepage()->saveBilling($addressData, $addressId);
    	$result = $this->getOnepage()->saveShippingMethod('freeshipping_freeshipping');
    	$this->getOnepage()->getQuote()->collectTotals()->save();
    	
    	$result = $this->getOnepage()->savePayment(array('method'=>'cashondelivery'));
    	$this->getOnepage()->saveOrder();
